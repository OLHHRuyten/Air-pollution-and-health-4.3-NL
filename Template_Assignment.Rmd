...
# install.packages("rnaturalearth")         Nog te installeren!!!
 install.packages("cbsodataR")           #  Nog te installeren!!!
```
...
# 1. Download de gedetailleerde gemeentekaart (bijv. van meest recente jaar)
# 'voorlopig = TRUE' zorgt ervoor dat je de nieuwste grenzen ophaalt
gemeentekaart <- cbs_get_maps(year = 2023, resolution = "hoog")

# 2. Inspecteer de kaart (bevat provincie-, gemeente- en landsdelen)
# We filteren hier op gemeenten niveau
gemeenten_sf <- gemeentekaart$gemeente

# 3. Plot de gedetailleerde kaart
ggplot(data = gemeenten_sf) +
  geom_sf(fill = "snow", color = "darkgray", size = 0.1) +
  theme_minimal() +
  labs(
    title = "Gedetailleerde Gemeentekaart Nederland",
    subtitle = "Ingebruikte CBS grenzen via cbsodataR"
  )
```
...
ggplot(data = Fijnstof_Regio, aes(x = Regio, y = as.numeric(`X2025`), fill = Regio)) +
  ...
  # 1. Teken de boxplots naast elkaar
  geom_boxplot(width = 0.5, alpha = 0.7,
               outlier.color = "red", outlier.shape = 16, outlier.size = 2) +
  
  # 2. Voeg de individuele meetstations als stipjes toe over de juiste boxplot
  geom_jitter(width = 0.15, alpha = 0.4, color = "grey30") +
  
  # 3. Geef beide groepen een eigen herkenbare kleur
  scale_fill_manual(values = c("Randstad" = "coral", "Niet-Randstad" = "skyblue")) +
  
  # 4. Titels en labels aanpassen
  labs(
    title = "Comparison Particulate matter emissions (2025)",
    subtitle = "Randstad versus Rest of the country",
    y = "PM2.5 Value (µg/m³)",
...
ggplot(BA_BronnenVanStikstof_long,
  ...
  # --- NIEUW: De dikke verticale lijn voor 2014 ---
